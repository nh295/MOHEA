...
%             [AEI,GD,HV,fHV,IGD] = getAllResults(strcat(path,filesep,'results'),selectors{j},creditDef{i},problemName{k});
...
%             save(strcat(problemName{k},'_',selectors{j},'_',creditDef{i},'.mat'),'res');
            ...
            title(problemName{k});
            hold on
            [data,labels,p,avg1,avg2] = runKWsignificance(path,selectors{j},creditDef{i},'Random',problemName{k});
            if p<0.05 && avg1<avg2
